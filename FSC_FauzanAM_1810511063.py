# -*- coding: utf-8 -*-
"""
Created on Sun Nov 22 10:53:45 2020
@author: recov
"""
...
#DataFrame = pd.read_csv('Kuesioner Rating Kualitas Merk Laptop (Responses) - Form Responses 1.csv', header=None)
#DataMatrix = data_clean_df.values
DataMatrix = np.array(data_clean_df).astype(float)
...
#ax.scatter(normalized_data_matrix[:, 0], normalized_data_matrix[:, 1], c=colors, alpha=0.1)
ax.scatter(normalized_data_matrix[:, 0], normalized_data_matrix[:, 1], normalized_data_matrix[:, 2], c=colors,
           alpha=0.1)
for idx, centroid in enumerate(centroids):
    ax.scatter(*centroids, c=colors_options[idx], s=150, marker='X', label='Cluster '+str(idx+1))
...
# melakukan filtering terhadap seluruh kriteria yang diterima maupun ditolak
current_max_value = max_potential_value
criteria = 1
while criteria and current_max_value:
    criteria = 0
    max_potential_vector = normalized_data_matrix[max_potential_index]  # nilai x1
    potential_ratio = current_max_value / max_potential_value  # nilai Pk and MaxPValue
...
for idx, centroid in enumerate(cluster_center):
    ax.scatter(*centroids, c='k', s=150, marker='X')
